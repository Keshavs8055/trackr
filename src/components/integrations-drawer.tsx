...
import { X, SlidersHorizontal, KeyRound, Loader2, Power, Trash2, Check, AlertCircle } from "lucide-react";
...
export function IntegrationsDrawer() {
  ...
  const handleSaveKey = async (providerId: string) => {
    ...
    try {
      ...
      setApiKeyInput("");
      setNotification("API key saved and encrypted successfully.");
      setTimeout(() => setNotification(null), 3000);
    } catch (err: any) {
      setErrorMessage(err?.userMessage || "Invalid API key or network error.");
    }
  };
  ...
  const handleConfirmDisconnect = async () => {
    ...
    try {
      ...
      setNotification("Provider disconnected and encrypted credentials removed.");
      setTimeout(() => setNotification(null), 3000);
    } catch (err: any) {
      setDisconnectTargetId(null);
            ...
            >
              ...
              <div className="p-5 overflow-y-auto space-y-4 flex-1">
                <p className="text-xs text-muted-foreground leading-relaxed">
                  Connect external public metadata APIs to enrich your resources. Trackr stores fetched metadata locally and encrypts API keys using Web Crypto AES-GCM 256-bit.
                </p>
                          ...
                          <div className="p-3 bg-secondary/30 rounded-lg border border-border/50 space-y-2.5">
                            <div className="flex items-center justify-between">
                              <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1">
                                <KeyRound className="size-3" /> Enter API Key
                              </span>
                              <button
                                onClick={() => {
                                  setConfiguringProviderId(null);
              ...
              <div className="p-4 bg-secondary/10 border-t border-border/30 text-center">
                <p className="text-[11px] text-muted-foreground/60">
                  Provider metadata is cached for performance and encrypted safely in your browser.
                </p>
              </div>
            </motion.div>
          </div>
